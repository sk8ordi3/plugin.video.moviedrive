...
class navigator:
    ...
    def getCategories(self):
        page = requests.get(f"{base_url}/filmek/")
    ...
    def getOnlyMovies(self):
        page = requests.get(f"{base_url}/filmek/?tag=&type=Film")
        soup = BeautifulSoup(page.text, 'html.parser')
    ...
    def getOnlySeries(self):
        page = requests.get(f"{base_url}/filmek/?p=1&type=Sorozat")
        soup = BeautifulSoup(page.text, 'html.parser')
    ...
    def getSeriesSources(self, url):
        ...
        for evad_div in evad_divs:
            ...
            if button:
                ...
                season_link = link
                response_2 = requests.get(season_link)
    ...
    def getEpisodes(self, url):
        ...
        if find_block:
            for episode in find_block['episodes']:
                ...
                #requests page_xx
                page_xx = requests.get(embed_link)
